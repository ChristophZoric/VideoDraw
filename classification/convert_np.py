...
for segment in coordinates:
    for i in range(len(segment) - 1):
        x1 = min(width - 1, int(segment[i][0] / max([pt[0] for pts in coordinates for pt in pts]) * width))
        y1 = min(height - 1, int(segment[i][1] / max([pt[1] for pts in coordinates for pt in pts]) * height))
        x2 = min(width - 1, int(segment[i + 1][0] / max([pt[0] for pts in coordinates for pt in pts]) * width))
        y2 = min(height - 1, int(segment[i + 1][1] / max([pt[1] for pts in coordinates for pt in pts]) * height))
